...
class ConnectorInstagramHistorico(Connector):
	...
	def execute(self):
		...
		for a in self.accounts:
			...
			insights = a.getDailyInsights(730)
			for i in insights:
			...
			insights = a.getLifetimeInsights(730)
			for i in insights:
				self.mongo.upsertDF(i.asRawDF(),'RAWDATA','InstagramInsights')
				self.sql.upsert(i.asSQLDF(),'PY_IG_AudienceInsight')

			logging.info('Recuperando seguidores online de : ' + a.instagram_name)
			insights = a.getOnlineFollowers(730)
			for i in insights:
