...
class DFPVideo(Mapping):
	...
	def sql(cls,df: pd.DataFrame):
		...
		for row in df.itertuples(index=False,name=None):
			AdsList = {}
			AdUnit = row[1].split('»')
			AdsList['UKEY'] = str(row[0]) + '_' + str(row[2]) + '_' + str(row[3]) + '_' + str(row[1])
			...
			AdsList['Position_of_pod'] = row[3]
			AdsList['Position_in_pod'] = row[2]
			AdsList['Ad_unit_code'] = row[4]
			AdsList['Ad_unit_reward_amount'] = None
			AdsList['Ad_unit_reward_type'] = None
			AdsList['Total_impressions'] = row[5]
			AdsList['Total_clicks'] = row[6]
			AdsList['fileid'] = None
